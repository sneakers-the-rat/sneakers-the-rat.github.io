...
```{r imports, results="hide", message=FALSE, warning=FALSE, toggle=TRUE}
library(tidyverse)
library(lme4)
library(rio)
library(binom)
...
```{r load, results="hide", message=FALSE, warning=FALSE, cache=TRUE, toggle=TRUE}
...
```{r, expt_1_1, cache=TRUE, dependson="load", toggle=TRUE}
...
```{r expt_1_2,cache=TRUE, dependson="load"}
...
```{r expt_1_3, toggle=TRUE}
e1_accs <- adjusted_accuracy(e1_image, e1_subject)
```

Only `r e1_accs$image`% of images were categorized above chance, and `r e1_accs$subject`% of subjects did, as opposed to the reported 94% and 98%, respectively.
...
```{r expt_2_1, cache=TRUE, dependson="load", toggle=TRUE}
...
```{r, cache=TRUE, dependson="load", toggle=TRUE}
...
Only `r e2_accs$image`% of images and `r e2_accs$subject`% of subjects classified above chance, as opposed to the reported 71% and 91%, respectively.

Collapsing across all images and subjects, only `r total_acc`% of responses agreed with the top category of the CNN.
...
Again, only `r e3a_acc$image`% of images and `r e3a_acc$subject`% of subjects were above chance accuracy of 1/48, as opposed to the reported 79% and 88%, respectively. Experiment 3b has qualitatively the same results, but their interpretation doesn't necessarily follow from the data:
...
Again, the corrected accuracies are much lower than they report, accounting for uncertainty, only `r e4_accs$image`% of images and `r e4_accs$subject`% of subjects had accuracies above chance, rather than the reported 100% and 81%, respectively.  This is exceptionally troubling for their interpretation of their results, as it is *subject* accuracy that matters, not *image* accuracy.
...
```{r, fig.height=12, cache=TRUE, dependson="load", toggle=TRUE}
...
This is reflected in the subject accuracy, where only `r e5_accs$subject`% of subjects had accuracies significantly better than chance.
...
Again, only `r e7_accs$image`% of images and `r e7_accs$subject`% of subjects had accuracy significantly above chance, as opposed to the reported 78% of images and 83% of subjects. Overall, across all images and subjects, the total accuracy was `r e7_total`%.
